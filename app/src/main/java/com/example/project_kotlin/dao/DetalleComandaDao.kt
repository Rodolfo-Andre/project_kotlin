...
interface DetalleComandaDao {
    ...
    fun obtenerTodo(): List<DetalleComanda>

    @Query("select * from Detalle_Comanda where comanda_id = :comanda_id")
    fun buscarDetallesPorComanda(comanda_id: Long): List<DetalleComanda>

    @Query("SELECT * FROM Detalle_Comanda WHERE comanda_id = :comanda_id AND id_plato = :id_plato")
    fun obtenerPorComandaYPlato(comanda_id: Long, id_plato: String) : DetalleComanda
